// Compass Firefox Extension - Popup Script
...
class PopupController {
    ...
    async checkConnection() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                connectionInfo.textContent = 'Firefox extension is tracking your browser activity';
            } else {
            ...
            connectionInfo.textContent = 'Cannot connect to Compass daemon. Make sure it\'s running on localhost:8080';
        }
    }
    ...
    async loadStats() {
        try {
            ...
            const stored = await browser.storage.local.get(['pendingEvents', 'dailyStats']);
    ...
    async updateCurrentTabTime() {
        try {
            const response = await browser.runtime.sendMessage({action: 'getCurrentTabTime'});
